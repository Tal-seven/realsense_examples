...
#include "/home/talha/librealsense/examples/example.hpp"          // Include short list of convenience functions for rendering
...
#include "/home/talha/realsense/imgui/imgui.h"
#include "/home/talha/realsense/imgui/imgui_impl_glfw.h"

// 3rd party header for writing png files
#define STB_IMAGE_WRITE_IMPLEMENTATION
#include "/home/talha/librealsense/third-party/stb_image_write.h"
...
void update_data(rs2::frame_queue& data, rs2::frame& depth, rs2::points& points, rs2::pointcloud& pc, glfw_state& view, rs2::colorizer& color_map, rs2::frame& f);
...
int main(int argc, char * argv[]) try
{
	...
	std::thread processing_thread([&]() {
		...
		{
			// ALIGN DATA
			rs2::frameset data_not_aligned = pipe.wait_for_frames(); // Wait for next set of frames from the camera
			auto data = align_to.process(data_not_aligned);
			// NOT ALIGN DATA
	...
	int file_number = 1;
	std::string file_number_str = "1";
	std::string file_name, image_name, image_crop_name, ovrlp_name;
	int selection = 0;
	int num_of_data_captured = 0;
	// Registration
	Mat img;
	rs2::frame frame_original;
	rs2::frame frame_post_processed;
	while (app)
	{
		...
		update_data(original_data, colored_depth, original_points, original_pc, original_view_orientation, color_map, frame_original);
		update_data(filtered_data, colored_filtered, filtered_points, filtered_pc, filtered_view_orientation, color_map, frame_post_processed);
		...
		selection = getch();
		if (selection == 32) {
			file_number_str = std::to_string(file_number);

			// Save Post Processed Point Cloud Data(.pcd)
			auto pcl_points = points_to_pcl(filtered_points);
			file_name = "post_processed_points_" + file_number_str + ".pcd";
			pcl::io::savePCDFileBinary(file_name, *pcl_points);
			
			// Save Original Point Cloud Data(.pcd)
			pcl_points = points_to_pcl(original_points);
			file_name = "original_points_" + file_number_str + ".pcd";
			pcl::io::savePCDFileBinary(file_name, *pcl_points);
			...
			if (color_data.poll_for_frame(&vf)) {
				// Write images to disk
				img = frame_to_mat(vf);
				image_name = "image_" + file_number_str + ".jpg";
				imwrite(image_name, img);

				cv::Rect customRect(300, 200, 500 - 300, 400 - 200);
				image_crop_name = "image_crop_" + file_number_str + ".jpg";
				imwrite(image_crop_name, img(customRect));
				std::cout << "Saved " << image_name << " and " << image_crop_name << std::endl;
			}
			file_number = file_number + 1;
			std::cout << std::endl;

			//// Finding Point Cloud of Part of Image Determined with Bounding Box[top_left_point(300, 200), bottom_right_point(500, 400)]
			float upixel[2];
			float upoint[3];
			rs2_intrinsics intr = frame_post_processed.get_profile().as<rs2::video_stream_profile>().get_intrinsics(); // Calibration data
			rs2::depth_frame depth_frame_used_in_deprojection = frame_post_processed;
			float udist;
			std::vector<std::array<float, 3>> ovrlppoints;
			std::array<float, 3> ovrlppoint;

			pcl::PointCloud<pcl::PointXYZ> ovrlppcloud;
			...
			for (int i = 300; i <= 500; i++)
			{
				for (int j = 200; j <= 400; j++)
				{
					...
					ovrlppcloud.push_back(tmp_point);
				}
			}

			ovrlp_name = "cropped_point_cloud" + file_number_str + ".pcd";
			pcl::io::savePCDFileBinary(ovrlp_name, ovrlppcloud);// *new pcl::PointCloud<pcl::PointXYZ>);
																//std::cout << "Saved " << "overlapping1.pcd" << std::endl;
																//show_cloud(*ovrlppcloud);
			std::cout << "Saved " << ovrlp_name << std::endl;
			std::cout << "DONE!!!";
...
void update_data(rs2::frame_queue& data, rs2::frame& colorized_depth, rs2::points& points, rs2::pointcloud& pc, glfw_state& view, rs2::colorizer& color_map, rs2::frame& f)
{
	//rs2::frame f;
